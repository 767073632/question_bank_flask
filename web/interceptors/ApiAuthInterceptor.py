...
def before_request_api():
    api_intercept_urls = app.config['API_INTERCEPT_URLS']
    ...
    pattern = re.compile('%s' % "|".join( api_intercept_urls ))
...
def check_member_login():
    ...
    try:
        member_info = Member.query.filter_by(id=int(auth_info[1])).first()
    except Exception:
        return False
